...
public class NhanVienTK_GUI extends JFrame implements ActionListener {
...
//	Khai báo màu
	...
	public NhanVienTK_GUI() throws Exception {
		try {
...
//		addColumn là thêm tiêu đè cột thôi
		...
		try {
			for (NhanVien nhanVien : nv.getAllNhanVien()) {
				...
				Object[] row = { nhanVien.getMaNV(), nhanVien.getTenNV(),nhanVien.getChucVu() ,nhanVien.getDiaChi(),gioitinh, nhanVien.getTuoi(), tienTeVN.format(nhanVien.getLuong()) };
				model.addRow(row);
			}
		} catch (Exception e) {
			e.printStackTrace();
...
public boolean kiemtra()
{
	...
	String chucvu = cmbBoPhan.getSelectedItem().toString();
...
{
	NhanVien nhanVien = new NhanVien(txtMaNV.getText(), txtTenNV.getText(),Integer.parseInt(txtTuoi.getText()),txtSDT.getText(),radNam.isSelected(),cmbBoPhan.getSelectedItem().toString(), 
